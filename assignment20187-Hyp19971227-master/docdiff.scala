...
def occurrences(xs: List[String]): Map[String, Int] = {
    xs.flatMap(s=>clean(s)).groupBy(identity).mapValues(_.size)
}

    /*val i = 0
    for( i <- xs.size){
        val dulplicateWord = clean(xs.toString).distinct.toString//string
    }
    val dulplicateTimes = xs.count( _ == dulplicateWord )//int
    List((dulplicateWord,dulplicateTimes)).toMap    
}*/
...
def prod(lst1: List[String], lst2: List[String]) : Int ={
    val dotProduct = occurrences(lst1).map { 
        case (k,v) => (k,v * occurrences(lst2).getOrElse(k,0))
    }
    dotProduct.values.sum
}
...
def overlap(lst1: List[String], lst2: List[String]) : Double ={
    val n = prod(lst1, lst2)
    val d = List(prod(lst1,lst1),prod(lst2,lst2)).max
    n.toDouble/d
}
