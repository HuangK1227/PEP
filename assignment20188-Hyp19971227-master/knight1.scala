...
def count_tours(dim: Int, path: Path) : Int = {
  ...
  else ( for (temp <- legal_moves(dim,path,path.head) ) yield count_tours(dim, temp::path)).sum
}
...
def enum_tours(dim: Int, path: Path) : List[Path] = {
  ...
  else ( for (temp <- legal_moves(dim,path,path.head) ) yield enum_tours(dim, temp::path)).flatten
}
...
def first_tour(dim: Int, path: Path) : Option[Path] = 
  ...
  else first(legal_moves(dim,path,path.head),f => first_tour(dim,f::path))
