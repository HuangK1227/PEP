...
def compute(prog: String, pc: Int, mp: Int, mem: Mem) : Mem = {
  if(pc >= prog.length) mem
  else prog.charAt(pc).toString match{
      ...
      case "[" => if(sread(mem, mp)!= 0) compute(prog, pc + 1, mp, mem)
                 else   compute(prog, jumpRight(prog,pc + 1, 0),mp,mem)
